...
int KWalletD::openInternal(const QString &wallet, const QString &appId)
{
    bool ok;
    ...
    return std::abs(int(rand.generate()));
}
...
int KWalletD::open(const QString &wallet, qlonglong wId, const QString &appId)
{
    ...
    int rnd = openInternal(wallet, appId);
    if (rnd < 0) {
        return -1;
    }
...
int KWalletD::openAsync(const QString &wallet, qlonglong wId, const QString &appId, bool handleSession)
{
    ...
    QTimer::singleShot(0, [this, wallet, appId, tid]() {
        int rnd = openInternal(wallet, appId);
        if (rnd < 0) {
            return;
        }
